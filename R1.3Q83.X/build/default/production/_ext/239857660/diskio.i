...
# 26 "../drivers/diskio.c" 2
# 1 "../drivers/../fatfs/ff.h" 1
# 29 "../drivers/../fatfs/ff.h"
# 1 "../drivers/../fatfs/../fatfs/ffconf.h" 1
# 29 "../drivers/../fatfs/ff.h" 2
# 49 "../drivers/../fatfs/ff.h"
...
typedef WORD WCHAR;
# 82 "../drivers/../fatfs/ff.h"
typedef DWORD FSIZE_t;
typedef DWORD LBA_t;
# 105 "../drivers/../fatfs/ff.h"
typedef char TCHAR;
# 132 "../drivers/../fatfs/ff.h"
typedef struct {
 BYTE fs_type;
 BYTE pdrv;
 BYTE ldrv;
 ...
 WORD n_rootdir;
 WORD csize;
# 152 "../drivers/../fatfs/ff.h"
 DWORD last_clst;
 DWORD free_clst;
# 163 "../drivers/../fatfs/ff.h"
 DWORD n_fatent;
 DWORD fsize;
 LBA_t volbase;
 LBA_t fatbase;
...
# 359 "../drivers/../fatfs/ff.h"
DWORD get_fattime (void);
# 27 "../drivers/diskio.c" 2
# 1 "../drivers/../fatfs/diskio.h" 1
# 13 "../drivers/../fatfs/diskio.h"
...
# 28 "../drivers/diskio.c" 2
# 1 "../drivers/../drivers/SDCard.h" 1
# 46 "../drivers/../drivers/SDCard.h"
...
# 30 "../drivers/diskio.c" 2
# 1 "../drivers/../drivers/fatdisk_debug.h" 1
# 27 "../drivers/../drivers/fatdisk_debug.h"
extern int fatdisk_debug(int debug);
# 31 "../drivers/diskio.c" 2



static int debug_flags = 0;
# 45 "../drivers/diskio.c"
static uint32_t start_lba = 0xffffffff;
...
DSTATUS disk_initialize(BYTE pdrv)
{
    ...
    do { if (debug_flags) printf ("disk_initialize(%d)...\n\r", pdrv); } while(0);
    if (pdrv != 0) {
        do { if (debug_flags) printf ("physical driver != 0\n\r"); } while(0);
        return 0x02;
    }


    do { if (debug_flags) printf ("read MBR partition table at sector 0\n\r"); } while(0);
    if (SDCard_read512(0, 384, buf, 128) != 0) {
        do { if (debug_flags) printf ("failed to read sector 0\n\r"); } while(0);
        return 0x02;
    } else {
        if (debug_flags)
    ...
    if (buf[126] != 0x55 || buf[127] != 0xaa) {
        do { if (debug_flags) printf ("no MBR signature\n\r"); } while(0);
        return 0x02;
    }
# 84 "../drivers/diskio.c"
    if (buf[66] != 0x01 && buf[66] != 0x04 && buf[66] != 0x06 &&
        buf[66] != 0x0b && buf[66] != 0x0c && buf[66] != 0x0e) {
        do { if (debug_flags) printf ("no FAT32 partition\n\r"); } while(0);
        return 0x02;
    }
    start_lba = buf[73];
    ...
    do { if (debug_flags) printf ("partition starts at sector %ld\n\r", start_lba); } while(0);
...
DSTATUS disk_status(BYTE pdrv)
{
    ...
    if (start_lba == 0xffffffff) {
        do { if (debug_flags) printf ("no valid volume\n\r"); } while(0);
        return 0x02;
    }
    return 0;
}
...
DRESULT disk_read(BYTE pdrv, BYTE* buff, LBA_t sector, UINT count)
{
    do { if (debug_flags & (1 << 1)) printf ("disk_read:  pdrv=%d, sector=%ld, count=%d\n\r", pdrv, sector, count); } while(0);

    for (int i = 0; i < count; i++) {
        if (SDCard_read512(start_lba + sector, 0, buff, 512) != 0) {
            do { if (debug_flags) printf ("failed to read sector %ld\n\r", sector); } while(0);
            return RES_ERROR;
        }
        if ((debug_flags & (1 << 1)) && (debug_flags & (1 << 3))) {
            util_addrdump("fat: ", sector * 512, buff, 512);
        }
        sector++;
        buff += 512;
    }
...
DRESULT disk_write(BYTE pdrv, const BYTE* buff, LBA_t sector, UINT count)
{
    do { if (debug_flags & (1 << 2)) printf ("disk_write: pdrv=%d, sector=%ld, count=%d\n\r", pdrv, sector, count); } while(0);

    for (int i = 0; i < count; i++) {
        if (SDCard_write512(start_lba + sector, 0, buff, 512) != 0) {
            do { if (debug_flags) printf ("failed to write sector %ld\n\r", sector); } while(0);
            return RES_ERROR;
        }
...
DRESULT disk_ioctl(BYTE pdrv, BYTE cmd, void* buff)
{
    ...
    do { if (debug_flags) printf ("disk_ioctl: pdrv=%d, cmd=%d, buff=0x%lx: res=%d\n\r", pdrv, cmd, (long)buff, res); } while(0);
    return res;
}
