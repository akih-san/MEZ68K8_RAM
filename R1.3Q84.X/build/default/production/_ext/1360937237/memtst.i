...
void mem_init()
{
    ...
    for (addr = 0; addr < 0x00080000; addr += 256 * 16) {
        printf("Memory 000000 - %06lXH\r", addr);
        tmp_buf[0][0] = (addr >> 0) & 0xff;
        tmp_buf[0][1] = (addr >> 8) & 0xff;
        tmp_buf[0][2] = (addr >> 16) & 0xff;

     write_sram(addr, tmp_buf[0], 256);
        read_sram(addr, tmp_buf[1], 256);

     if (memcmp(tmp_buf[0], tmp_buf[1], 256) != 0) {
            printf("\nMemory error at %06lXH\n\r", addr);
            util_addrdump("WR: ", addr, tmp_buf[0], 256);
            util_addrdump("RD: ", addr, tmp_buf[1], 256);
   while(1){};
            break;
        }
        if (addr == 0) continue;

     read_sram(0, tmp_buf[1], 256);
        if (memcmp(tmp_buf[0], tmp_buf[1], 256) == 0) {
