...
# 2 "<built-in>" 2
# 1 "../src/mez68k8_main.c" 2
# 20 "../src/mez68k8_main.c"
# 1 "../src/../src/mez68k8.h" 1
# 26 "../src/../src/mez68k8.h"
# 1 "../src/../src/picconfig.h" 1
# 23 "../src/../src/picconfig.h"
...
void main(void)
{
 ...
 switch (selection) {
  case 1:
   fileinfo.fname[0] = 0;
   c = load_program((uint8_t *)unimon, 0x0000);
   break;
  case 2:
   fileinfo.fname[0] = 0;
   c = load_program((uint8_t *)basic, 0x0000);
   break;
  default:
   if ( !c ) {
...
static int load_program(uint8_t *fname, uint32_t load_adr) {
 ...
 rdbuf = (void *)&tmp_buf[0][0];

 sprintf((char *)buf, "%s", fname);

 fr = f_open(&rom_fl, buf, 0x01);
 if ( fr != FR_OK ) return((int)fr);

 adr = load_adr;
 cnt = size = (uint16_t)((&rom_fl)->obj.objsize);
 btr = 256 * 2;
 while( cnt ) {
...
static int open_dskimg(void) {
 ...
 for (drv = num_files = 0; drv < 16 && num_files < 4; drv++) {
   ...
   if (cpm_drives[0].filep == ((void*)0)) return -4;
         num_files++;
        }
    }
    return 0;
...
static void setup_cpm(void) {
 ...
 flg = load_program((uint8_t *)buf, 0x0000);
 if (!flg) {
  sprintf((char *)buf, "%s/%s", fileinfo.fname, cbios);
  flg = load_program((uint8_t *)buf, 0x6200);
 }
 if ( flg ) {
  printf("Program File Load Error.\r\n");
  while(1);
